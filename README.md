...
* **`Main.java`**:
    * Contiene el método `main()` para demostrar el uso de `PrimesList` y `PrimeGenerator`.
    * Muestra cómo se manipula la `PrimesList` (añadir, remover, etc.) y cómo múltiples `PrimeGenerator` pueden generar primos concurrentemente, añadiéndolos a una única `PrimesList`.

* **`PrimesListTest.java`**:
    * Clase de pruebas unitarias JUnit 5 para `PrimesList`.
